...
/** Public quote API — no admin bearer required. */
export const postGetQuotesProxy: RequestHandler = async (req, res) => {
  try {
    const response = await fetch(`${getAdminApiBaseUrl()}/api/v1/quotes/get-quotes`, {
      method: "POST",
      headers: {
    ...
    res.status(500).json({ detail: "Failed to fetch quotes" });
  }
};
